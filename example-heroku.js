...
app.get("/stats", function(req, res) {
	...
	console.log("Stats retrieved by: "+req.ips);
});
...
var pingself = function(self){
	...
	request.get({method:'GET',encoding:'utf8',uri:self+"/ping",json:true,followAllRedirects:true}, function(error, response, body) {
		if(error) console.log(error);
		else console.log(body);
	});
}

setInterval(function(){pingself(process.env.APP_URL||"http://steamchatbot.herokuapp.com")},5000);

getTime = function() {
	var meminfo = process.memoryUsage();
	var curtime = process.hrtime(this.startTime);
	curtime = curtime[1] + curtime[0]*1000000000;
	var message  = "I have been running for about " + _nanosecondsToStr(Math.floor(curtime/1000000000, true));
