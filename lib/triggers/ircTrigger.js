...
options: {
  ...
  defaults: { //default settings everywhere
    ircOptions: {                //this object will be passed into the irc object without checking it, other than sane defaults for the specific items below. Full options are available at https://node-irc.readthedocs.org/en/latest/API.html#internal
      ...
      messageSplit: 256,	//I believe the maximum length RFC 1459 allows for a message (including prefixes like command, target, CRLF at end, etc) is 512 characters. You probably don't want this any higher than 256 though, long messages are annoying.
    ...
    steamFormat: { //this is optional, the arguments shown below are defaults; false means don't send the message. What messages do we want to tell steam users about? The following will be replaced where appropriate: $nick $user $host ($nick!$user@$host) $channel $message $server (internal server name) $msgprefix $sep (defined above) $by (the person that did the kicking) You may want to define this before you initialize your bot, then just pass in the object here.
  ...
  relays: [ //You need an entry for each channel that you want to relay. You can relay a chat to multiple networks if you like, but you need multiple entries for it, and it will *not* relay from one chat to another.
    {
...
IRCRelay.prototype._onLoggedOff = function() {
	var that = this;
	setTimeout(function(){
		if(this.chatBot.steamClient.loggedOn) { return; }
		_.each(that.servers, function(server) {
			that._tryCallback(true,function(){
	...
	try {
		...
		_.each(that.options.relays, function(link) {
			...
			if(link.server===server.name && link.channel && link.steamchat) {
				...
				that.winston.debug("validlink: ",validOptions);
			}
		});
		...
		ircServer.connectfunc = function(){
			if(server.performs && server.performs.length > 0) {
				...
				_.each(server.performs, function(line) {
					that.winston.info("Sending command: "+line);
					try {
						ircServer.send(line);
					} catch(err) {
						that.winston.error(err.stack);
...
IRCRelay.prototype._configureRelays = function(server) {
	...
	server.addListener('message#', function(nick, to, text, message) {
		that.winston.silly("message: ",message);
		that.winston.silly("server: ",server);
		_.each(server.relays, function(link) { that._tryCallback(function(){
				if(to.toLowerCase()===link.channel.toLowerCase() && nick !== link.ircServer.nick) {
					...
					that.winston.log(msg);
					if(msg) {
						that._sendMessageAfterDelay(link.steamchat,msg);
					}
				}
		});	});
	});
	server.addListener('notice', function(nick, to, text, message) {
		...
		_.each(server.relays, function(link) { that._tryCallback(function(){
			if(to.toLowerCase()===link.channel.toLowerCase() && nick !== link.ircServer.nick) {
				...
				if(msg) { that._sendMessageAfterDelay(link.steamchat,msg); }
				if(msg) sent=true;
			}
		}); });
		...
		_.each(admins, function(admin) { that._tryCallback(function(){
			if(nick !== link.ircServer.nick) {
				...
				if(msg) { that._sendMessageAfterDelay(admin,msg); }
			}
		}); });
	});
	...
	server.addListener('error', function(error) {
		var admins = server.admins || that.admins || false;
		_.each(admins, function(admin) { that._tryCallback(function(){
			that._sendMessageAfterDelay(admin,JSON.stringify(error));
		}); });
		that.winston.error(error);
	});
	return true;
...
//	}
	...
	var sent = false;
	that.winston.silly(message);
	_.each(this.relays, function(link) {
		that.winston.silly(link.ircServer.name);
