...
var processRooms = function(input) {
	...
	for(var chat in rooms) {
		...
		var groupid = document.createElement('span');
			...
			$(groupid).click(function(e){e.stopPropagation();$('.groupid').val(chat)});
...
//		group = document.createElement('td');
		...
		for(var user in rooms[chat]) {
			...
			td.textContent=user;
			$(td).click(function(e){e.stopPropagation();$('.userid').val(user)});
			td.className='GroupUserID';
			tr.appendChild(td);

			td = document.createElement('td');
			td.textContent=ranks[rooms[chat][user].rank];
			td.className='GroupUserRank';
			tr.appendChild(td);

			td = document.createElement('td');
			td.textContent=rooms[chat][user].permissions; //parse the permissions later, create buttons to kick etc, or maybe just copy the values to the form.
