$(document).ready(function(){
	...
	success.hide();
	error.hide();
	$('#unapprovedTitle').click(function(){$('#unapprovedTable').toggle()});
	$('#approvedTitle').click(function(){$('#approvedTable').toggle()});
	$('#usedTitle').click(function(){$('#usedTable').toggle()});
	...
	var displayResult = function(data) {
		...
		if(data.keys) {
			document.getElementById('usedBody').innerHTML="";
			document.getElementById('approvedBody').innerHTML="";
			document.getElementById('unapprovedBody').innerHTML="";
	...
	var addRow = function(data) {
		...
		if(!data.usedby) {
			...
			publicCont.appendChild(public);
			var label = document.createElement('label');
			label.setAttribute('for',data.addedtime+'public');
//			label.textContent='Drop in public?';
//			$(label).hide();
			publicCont.appendChild(label);
			controls.appendChild(publicCont);
			controls.appendChild(document.createTextNode(" "));
		...
		if(data.usedby) {
			document.getElementById('usedBody').appendChild(row);
		} else if(data.approvedby) {
			document.getElementById('approvedBody').appendChild(row);
		} else {
			document.getElementById('unapprovedBody').appendChild(row);
		}
	}
});
