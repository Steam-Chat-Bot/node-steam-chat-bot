...
var YoutubeTrigger = function() {
	YoutubeTrigger.super_.call(this);
};
...
YoutubeTrigger.prototype._respond = function(toId, message) {
	...
	if (query) {
		var that = this;
		youtube.feeds.videos({ q: query, 'max-results': 1 }, function(err, data) {
			if (err instanceof Error) {
				winston.error("Error querying youtube: " + err.message);
				...
				if (data && data.items && data.items.length > 0 && data.items[0].id) {
					...
					winston.warn("Bad data from youtube", data);
					that._sendMessageAfterDelay(toId, "¯\\_(ツ)_/¯");
				}
			}
		});
...
exports.create = function(name, chatBot, options) {
	var trigger = new YoutubeTrigger();
