...
SteamrepTrigger.prototype._respond = function(toId, message) {
	...
	if (steamid) {
		var that = this;
		var steamrep={};
		that.winston.info("Checking steamrep.com for " + steamid);
		request.get({method:'GET',encoding:'utf8',uri:'http://steamrep.com/api/beta/reputation/'+steamid+'?json=1&extended=1',json:true,followAllRedirects:true}, function(error, response, body) {
			if (error) {
				...
				that.winston.warn("Code " + response.statusCode + " from steamrep for steamid " + steamid);
				return;
			}
			steamrep = body;
...
SteamrepTrigger.prototype._getParsedResult = function(steamrep) {
	if (steamrep.steamrep) {
		...
		} else if(status=="notfound") return "This user has not yet been added to steamrep's database. To add them, please visit "+url64;
