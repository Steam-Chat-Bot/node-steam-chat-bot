...
RandomGameTrigger.prototype._respond = function(roomId, userId, message) {
	var query = this._stripCommand(message); var steamId;
	if(query) {
		steamId = query.params[1] || userId;
		if(!this.options.apikey) {
			this.winston.error("No api key!");
			this._sendMessageAfterDelay("I don't have an API key!");
			return false;
		}
	} else {
		return false;
	...
	var that = this;
	winston.info("Fetching a random game for " + steamId);
	var fullurl = "http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key="+that.options.apikey+"&steamid="+steamId+"&include_played_free_games=1&include_appinfo=1&format=json";
	request.get({method:"GET",encoding:"utf8",uri:fullurl,json:true,followAllRedirects:true}, function(error, response, body) {
		if (error) {
			...
			that._sendMessageAfterDelay("I'm sorry, I got an error from the server!");
			return;
		}
		try {
			...
			that._sendMessageAfterDelay(roomId, game.name+". Click steam://run/"+game.appid+" to play it, or http://steamcommunity.com/app/"+game.appid+" to view the community page."+(time ? " You have spent "+time+" playing it." : ""));
		} catch(err) {
			that.winston.error(err.stack);
			that._sendMessageAfterDelay(roomId, "Error obtaining games list");
		}
	});
	return true;
}
