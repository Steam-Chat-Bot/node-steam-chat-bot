...
MoneyTrigger.prototype._getCurrencies = function(toId, userId, params) {
	...
	if(!cache.get("currencies") || !money.base || !money.rates) {
		...
		request.get({method:"GET",encoding:"utf8",uri:"http://openexchangerates.org/api/currencies.json?app_id="+that.options.apikey,json:true,followAllRedirects:true}, function(error, response, data) {
			if (error) {
				try{
		...
		if(params[1] && data[params[1]]) {
			...
			that._sendMessageAfterDelay(userId, "To get the full name of a currency, put it after the command. Valid currencies are: '"+Object.keys(data).join("', '")+"'");
			if(toId!==userId) that._sendMessageAfterDelay(toId, "To get the full name of a currency, put it after the command. Use this command in private to get a list of currencies.");
		}
	}
	return true;
}
