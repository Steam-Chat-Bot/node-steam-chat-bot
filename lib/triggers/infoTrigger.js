...
InfoTrigger.prototype._nanosecondsToStr = function(seconds, goagain) {
    ...
    if(temp > 259200) {
        ...
        var next = (goagain==true ? this._nanosecondsToStr(seconds-temp*86400,false) : "");
        return " " + temp + " day" + numberEnding(temp) + next;
    } else if (temp > 10800) {
        var temp = Math.floor(temp / 3600);
        var next = (goagain==true ? this._nanosecondsToStr(seconds-temp*3600,false) : "");
        return " " + temp + " hour" + numberEnding(temp) + next;
    } else if (temp > 180) {
        var temp = Math.floor(temp / 60);
        var next = (goagain==true ? this._nanosecondsToStr(seconds-temp*60,false) : "");
