...
exports.create = function(name, chatBot, options) {
		...
		trigger.options.command = trigger.options.command || "!lock";
...
LockChatTrigger.prototype._respond = function(roomId, userId, message) {
		...
		if(query) {
			if (this.options.privatemessage && query != roomId)
				this.chatBot.steamClient.sendMessage(query, "I have been told to lock this chat by " + this.chatBot.steamClient.users[userId].playerName + " (" + userId + ").");
			this.chatBot.steamClient.lockChat(query);
			return true;
		} else {
			if(this.options.publicmessage)	this.chatBot.steamClient.sendMessage(roomId, this.options.publicmessage);
			this.chatBot.steamClient.lockChat(roomId);
			return true;
		}
		return false;
}
