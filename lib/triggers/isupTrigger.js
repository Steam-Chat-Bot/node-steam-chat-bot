...
isupTrigger.prototype._respond = function(toId, message) {
	...
	var that = this;
	if (query) {
		if(query.indexOf("://") == -1) query = "http://" + query;
		try {
			request(query, function (error, response, body) {
				if (!error) {
					if(response.statusCode==200 || response.statusCode==301)
		                                that._sendMessageAfterDelay(toId, "It's working just fine for me.");
					else that._sendMessageAfterDelay(toId, "The server is running, but it returned http status code "+ response.statusCode+". Please look it up for more information.");
				} else {
					console.log("ERROR! " + error);
					try{that.sendMessageAfterDelay(toId, "ERROR! Please notify the bot admin to check the logs!");
					} catch(e) { console.log(e.stack); that._sendMessageAfterDelay(toId, "ERROR!"); }
				}
			})
			return true;
