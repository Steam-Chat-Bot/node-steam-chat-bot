	...
	if (query) {
		...
		images.search(query, function(err, results) {
			if (err || !results || results.length < 1) {
				that.winston.error(that.chatBot.name+"/"+that.name+": Error querying google images or no links found: " + err.stack);
				that._sendMessageAfterDelay(toId, err ? err.message : "No results found");
			}
			else {
