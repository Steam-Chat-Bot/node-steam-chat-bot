...
InfobotTrigger.prototype._respond = function(toId,userId,message,options) {
	var that = this;
	var msg = that._stripCommand(message, that.options.cmdDelete);
	if(msg && (that.options.userlearn==true || that.options.admin.indexOf(userId) > -1)) {
		that._dbOperation("get",{key: msg.pair[0].toLowerCase(), callback: function(err,value){
			if(value && JSON.parse(value).locked==true) {
				...
				that._dbOperation("remove", function(err){
					if(!err) {
	...
	msg = that._stripCommand(message, that.options.cmdTell);
	if(msg || (options && options.see)) {
		that._dbOperation("get",{key:((options && options.see && options.counter < 5)?options.see : msg.pair[0]),callback: function(err, value){
			if(value) {
				...
				if(factoid.delim==' is see ') {
					that._respond(toId,userId,{see:factoid.definition,counter:((options && options.counter)?options.counter+1:0)});
					return true;
				} else {
					that.winston.debug(factoid.definition.length+" factoids available");
	...
	msg = that._stripCommand(message, that.options.cmdLock);
	if(msg && (!that.options.admin || (that.options.admin.indexOf(userId) > -1))) {
		that._dbOperation("get",{key:msg.pair.splice[0],callback: function(err, value){
			if(!value) {
	...
	if(!this.options.cmdLearn) msg = this._stripCommand('!something stupid'+message,'blah');
	else msg = this._stripCommand(message, this.options.cmdLearn);
	if(msg && msg.pair.length==2 && (that.options.userlearn==true || that.options.admin.indexOf(userId) > -1)) {
		...
		that._dbOperation("get",{key: msg.pair[0].toLowerCase(),callback:function(err, value){
			if(!err && value && JSON.parse(value).locked==true && that.options.cmdLearn) {
				...
				if(that.options.cmdLearn) that._sendMessageAfterDelay(toId, "Learned: "+msg.pair[0]+msg.delim+msg.pair[1]);
			}
		}});
		return true;
	}
