...
exports.create = function(name, chatBot, options) {
	...
	trigger.options.sendExtras = trigger.options.sendExtras || false; //send joins/parts and such. can be overridden with sendSteamExtras or sendDiscordExtras
	trigger.options.sendDiscordExtras = trigger.options.hasOwnProperty('sendDiscordExtras') ? trigger.options.sendDiscordExtras : trigger.options.sendExtras;
	trigger.options.sendUserUpdates = trigger.options.hasOwnProperty('sendUserUpdates') ? trigger.options.sendUserUpdates : trigger.options.sendExtras; //if you only want name changes and not joins/parts (only works for changes on discord, not steam)
	trigger.options.sendsendSteamExtras = trigger.options.hasOwnProperty('sendSteamExtras') ? trigger.options.sendSteamExtras : trigger.options.sendExtras;
	return trigger;
}
...
DiscordRelay.prototype._onLoggedOn = function() {
	...
	this.discord.on('userUpdate', function(user, oldUser) {
		if((that.options.sendDiscordExtras||that.options.sendUserUpdates) && user.username !== oldUser.username && (that.options.sendDiscrims||that.options.sendUserIds)) {
...
DiscordRelay.prototype._respondToChatMessage = function(roomId, chatterId, message) {
	if(roomId!==this.options.steamChat) {
		return false;
	}
	var that = this;
...
DiscordRelay.prototype._sendSteamAction = function(action,userId,byId) {
	byId = byId || null;
	var that = this;
	if(that.discordOn && that.options.sendSteamExtras) {
		this.discord.createMessage(that.options.discordChannelID,
