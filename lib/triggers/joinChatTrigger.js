...
JoinChatTrigger.prototype._respond = function(roomId, userId, message) {
		...
		if (query) {
				this.chatBot.steamClient.joinChat(query);
				if(this.options.notify)
					this.chatBot.steamClient.sendMessage(query, "Hello. I was commanded to join this chat by " + this.chatBot.steamClient.users[userId].playerName + " (" + userId + ").");
				return true;
		}
		return false;
}
