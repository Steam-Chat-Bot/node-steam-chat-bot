...
exports.create = function(name, chatBot, options) {
	...
	trigger.options.password = trigger.options.password || undefined;
	trigger.options.algorithm = trigger.options.algorithm || 'aes-256-crt';
	
	trigger.scope = 'wallet:accounts:read,wallet:sells:create,wallet:buys:create';
	trigger.db = (function() { try { return JSON.parse(this.decrypt(fs.readFileSync(trigger.options.dbFile))); } catch(e) { return {}}})();
...
BitcoinTrigger.prototype._onLoad = function() {
	...
	if (this.options.saveTimer !== -1) {
		setInterval(() => {
			try {
				fs.writeFile(that.options.dbFile, this.encrypt(JSON.stringify(that.db)), function (e) {
					if (e) {
	...
	if(!this.options.password || !this.options.encryption) {
		this.winston.error(withName + 'Must specify password and encryption!');
		return false;
	}
	else {
		return true;
...
BitcoinTrigger.prototype.encrypt = function(text) {
	var cipher = crypto.createCipher(this.options.algorithm, this.options.password);
	var crypted = cipher.update(text, 'utf8', 'hex');
	crypted += cipher.final('hex');
	return crypted;
}

BitcoinTrigger.prototype.decrypt = function(file) {
	var text = fs.readFileSync(file, { encoding: 'utf8' });
	var decipher = crypto.createDecipher(this.options.algorithm, this.options.password);
