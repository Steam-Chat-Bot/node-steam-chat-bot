...
pingTimer     = int       - 0                           - How often should the server ping the client? Nginx defaults to closing connections after 60s of inactivity, so 30-50s is probably good. Alternatively, configure your nginx properly and this isn't needed.
*/
...
exports.create = function(name, chatBot, options) {
	...
	trigger.options.pingTimer     = options.pingTimer     || 0;
...
LogTrigger.prototype._respondToFriendMessage = function(userId, message) {
	...
	if(that.options.logUserChats||that.options.logConsole) { // log user messages
		that.winston.debug(this.chatBot.name+"/"+this.name+": Logging message from "+userId);
		if(!(that.logs.users[userId] && that.logs.users[userId].log instanceof Function)) {
			...
			that.winston.info(this.chatBot.name+"/"+this.name+": Initiating user log "+filename);
		...
		that.logs.users[userId].info(that._timestamp() + that._username(userId) + that._message(message),function(){
			that.winston.debug(this.chatBot.name+"/"+this.name+": logged user message");
		});
	}
	if(this.options.logGlobal) {
...
LogTrigger.prototype._respondToChatMessage = function(roomId, userId, message) {
	...
	if(this.options.logGroupChats === true||this.options.logConsole) {
		...
		if(!(this.logs.groups[roomId] && this.logs.groups[roomId].log instanceof Function)) {
			...
			that.winston.info(this.chatBot.name+"/"+this.name+": "Initiating group log "+roomId);
		...
		this.logs.groups[roomId].info(this._timestamp() + this._username(userId) + this._message(message), function(){
			that.winston.debug(this.chatBot.name+"/"+this.name+": logged groupchat message")
		});
	};
	if(this.options.logGlobal) {
...
LogTrigger.prototype._global = function(roomId, userId, message, type, actedId) {
	...
	if(that.options.logGlobal && !(this.logs.global && this.logs.global.info instanceof Function)) {
		that.winston.info(this.chatBot.name+"/"+this.name+": Initializing global logfile");
	...
	this.logs.global.info(message,log,function(){
		that.winston.debug(this.chatBot.name+"/"+this.name+": logged to global log");
	});
...
LogTrigger.prototype._startServer = function(){
	...
	this.express.get(this.options.liveURL, function(req, res){
		that.winston.info(this.chatBot.name+"/"+this.name+": Logfile request by " + that.chatBot._getClientIp(req)); //don't define crap in two places...
	...
	this.winston.info(this.chatBot.name+"/"+this.name+": websocket server created")
	this.wss.connections = {}; //{name: "value"};
	this.wss.connum =0;
	this.wss.on("connection", function(ws) {
		that.winston.info(this.chatBot.name+"/"+this.name+": "+(ws.upgradeReq.headers["x-forwarded-for"] || ws.upgradeReq.connection.remoteAddress)+" connected via websocket");
	}
	this.wss.on("open", function(ws) {
		that.winston.info(this.chatBot.name+"/"+this.name+": "+(ws.upgradeReq.headers["x-forwarded-for"] || ws.upgradeReq.connection.remoteAddress)+" websocket connection is open");
...
//			log.stream = true; //eventually I can do something about the bad ordering caused by people talking while this is still going on
		...
		that.winston.debug(this.chatBot.name+"/"+this.name+": websocket connection open")
		...
		ws.on("close", function() {
			...
			that.winston.debug(this.chatBot.name+"/"+this.name+": websocket connection closed");
		});
		ws.on("error", function() {
			...
			delete that.wss.connections[ws.id];
			that.winston.debug(this.chatBot.name+"/"+this.name+": websocket connection error. probably just someone closing the page. on('close') doesn't work right. BOO.");
		});

		if(that.options.pingTimer) {
			ws.pingTimer = setInterval(function(){
				ws.send("{type:ping,time:"+(new Date()-0)+"}");
...
LogTrigger.prototype._sendSockets = function(log) {
	var that = this;
	that.winston.silly(this.chatBot.name+"/"+this.name+": "+log);
