...
LogTrigger.prototype._respondToChatMessage = function(roomId, userId, message) {
	...
	if(this.options.logGroupChats === true||this.options.logConsole) {
		...
		if(!(this.logs.groups[roomId] && this.logs.groups[roomId].log instanceof Function)) {
			...
			that.winston.info(that.chatBot.name+"/"+that.name+": Initiating group log "+roomId);
...
LogTrigger.prototype._startServer = function(){
	...
	this.wss.on("connection", function(ws) {
		that.winston.info(that.chatBot.name+"/"+that.name+": "+(ws.upgradeReq.headers["x-forwarded-for"] || ws.upgradeReq.connection.remoteAddress)+" connected via websocket");
	});
	this.wss.on("open", function(ws) {
		that.winston.info(that.chatBot.name+"/"+that.name+": "+(ws.upgradeReq.headers["x-forwarded-for"] || ws.upgradeReq.connection.remoteAddress)+" websocket connection is open");
...
//			log.stream = true; //eventually I can do something about the bad ordering caused by people talking while this is still going on
		...
		ws.on("close", function() {
			...
			that.winston.debug(that.chatBot.name+"/"+that.name+": websocket connection closed");
		});
		ws.on("error", function() {
			...
			delete that.wss.connections[ws.id];
			that.winston.debug(that.chatBot.name+"/"+that.name+": websocket connection error. probably just someone closing the page. on('close') doesn't work right. BOO.");
		});
