...
pingTimer     = int         - Defaults to 0                           - How often should the server ping the client? Nginx defaults to closing connections after 60s of inactivity, so 30-50s is probably good. Alternatively, configure your nginx properly and this isn't needed.
*/
...
//			log.stream = true; //eventually I can do something about the bad ordering caused by people talking while this is still going on
			if(!query.room || query.room === log.roomId) try { ws.send(JSON.stringify(log)) } catch(err){that.winston.error(err.stack)};
		});
		that.winston.debug("websocket connection open")

		ws.on("close", function() {
			if(ws.pingTimer) clearTimeout(ws.pingTimer);
