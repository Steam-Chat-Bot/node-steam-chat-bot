...
LogTrigger.prototype._global = function(roomId, userId, message, type, actedId) {
	...
	var log = {
		...
		actedName:(actedId==null ? null : ((that.chatBot.steamClient.users && actedId in that.chatBot.steamClient.users) ? (that.chatBot.steamClient.users[actedId].playerName) : steamId) + " "),
		message:message
	}
...
LogTrigger.prototype._startServer = function(){
	var that = this;
	that.linesToSend = that.options.linesToSend;
	...
	this.wss.on("connection", function(ws) {
		...
		var linesToSkip = that.lineCount - that.linesToSend;
