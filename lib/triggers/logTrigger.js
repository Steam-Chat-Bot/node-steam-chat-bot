...
web           = string    - "/logs"                     - Should we enable the webserver portion? Live logs and logfiles. Set to false to disable. Set to true to not prefix logs with anything. Otherwise set as a url path prefix for this plugin.
liveURL       = string    - "/live"                     - where do you want to serve the "live" log (stuff shows up as it happens). This will be prefixed unless above is actually 'true'
logURL        = string    - "/files"                    - where do you want to serve the logfiles (static .txt files). route comes before this. This will be prefixed unless above is actually 'true'.
...
defaultStyle  = string    - "standard.css"              - Name of a file from the logTriggers css folder. This will be the default style applied to the live logs.
*/
...
exports.create = function(name, chatBot, options) {
	...
	trigger.options.defaultStyle  = options.defaultStyle  || "standard.css";
...
LogTrigger.prototype._startServer = function(){
	...
	this.winston.info('route: ',this.options.web);
...
//		res.send(fs.readFileSync(__dirname+'/logTrigger/index.html'));
...
*/
	...
	this.sockets.on("connection", function(socket) {
		...
		socket.emit('styles',styles);
		socket.on("options", function(data) {
			that.winston.info("options received");
			var linesToSend;
			if(data && data.lines) {
