...
serverPort    = string    - "false"                     - start a webserver on the specified port, if given, and serve logs from it, live, at /logs#gid where gid is a group's steamid (or leave out the hash for a log of all chat). Requires logGlobal enabled.
...
exports.create = function(name, chatBot, options) {
	...
	trigger.options.logDir        = options.logDir        || ("./logs/"+chatBot.username);
...
LogTrigger.prototype._onLoad = function(){
	console.log("onload");
	if(this.webServer !==  false && !this.express) {
		this._startServer();
	}
...
LogTrigger.prototype._respondToBan = function(roomId,bannedId,bannerId) {
	if(this.options.logGlobal) {
		this._global(roomId, userId, "has been banned","ban",bannerId);
	}
	return false;
}
...
LogTrigger.prototype._respondToFriendMessage = function(userId, message) {
	...
	if(this.options.webServer!==false && !this.express) {
		this._startServer();
	}
	if(this.options.logGlobal) {
...
console.log("starting server");
	...
	if(!this.options.express) {
		this.server.listen(that.options.serverPort, function() {
