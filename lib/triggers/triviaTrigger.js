...
TriviaTrigger.prototype._respond = function(toId, userId, message) {
	...
	if(query && this.admins.indexOf(userId) > -1) {
		if(query.toLowerCase()==="question") {
			...
			this._sendMessageAfterDelay(toId,"Valid commands: '$1 question $QUESTION||$ANSWER', '$1 admin', '$1 admin add $STEAMID64', '$1 admin del $#', '$1 admin del $STEAMID64', '$1 say $SOMETHING', '$1 answered', '$1 mod', '$1 unmod'".split("$1").join(this.options.command))
		}
		return true;
	} else if(toId!==userId && this.answer) {
		_.each(this.answer, function(possibleAnswer) {
			if(possibleAnswer.indexOf(message.toLowerCase())>-1) {
				if(message.length < 5 && message!==possibleAnswer) {
