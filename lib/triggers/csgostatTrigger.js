	...
	if(query && query.params.length === 2) {
		this.chatBot.steamApi('ISteamUserStats', 'GetUserStatsForGame', 2, 'get', this.options.apiKey, {
			...
			try {
				...
				this._sendMessageAfterDelay(toId, `${this.chatBot.steamFriends.personaStates[body.playerstats.steamID].player_name} ` +
					`(${body.playerstats.steamID}) has ${totalKills} kills and ${totalDeaths} deaths (${kdRatio.toFixed(2)}), accuracy ` +
					`of ${accuracy.toFixed(2)}, win percentage of ${winRatio.toFixed(3)*100}%, total playtime of ${hours.toFixed(0)} ` +
					`hours and ${minutes.toFixed(0)} minutes, ${headshots} headshots, and has broken ${windows} windows.`);
			}
			catch(e) {
