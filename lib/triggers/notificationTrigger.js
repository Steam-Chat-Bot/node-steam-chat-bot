...
NotificationTrigger.prototype._onLoad = function() {
	...
	if(this.options.nodemailer) {
		...
		try {
			...
			if(that.options.debug){ nodemailer.on('log', function(item){ that.winston.debug(item); }); }
		} catch(err) {
	...
	if(that.options.saveTimer !== -1) {
		var timerid = setInterval(function(){
			try {
				that.winston.silly(that.name+": Writing database to disk");
	...
	if(this._stripCommand(message,this.options.seenCommand)) {
		...
		if(line === "delete yes") {
			...
			pusher.devices(function(error, response){
				that.winston.silly(response.toString());
				if (error) {
				...
				if(error) {
					...
					for(x=0;x<response.devices.length;x++) {
						...
						if(devices[x].iden===iden) {
							...
							that.winston.debug(that.name+" sending pushbullet device confirmation for "+that._username(userId)+"/"+userId);
			...
			pusher.me(function(err,response){
				if(err) {
					...
					var pusher=new PushBullet(apikey);
					that.winston.debug(that.name+" sending pushbullet confirmation for "+that._username(userId)+"/"+userId);
			...
			var msg = {
				...
				url: "https://github.com/Efreak/node-steam-chat-bot/issues/new",
				url_title: "Submit issue to developer"
			}
			that.winston.debug(that.name+" sending pushover confirmation for "+that._username(userId)+"/"+userId);
			pusher.send(msg,function(err, result) {
				if(err) {
					onerror(err);
				} else {
					that.winston.error("Please create an issue with the following information so I can complete this feature. This is the results of what happens when pushbullet sends a push");
			...
			that.winston.silly('input: '+email);
			that.winston.silly('propo: '+that.hashfunc(that.db[userId].emailProposed));
			...
			that.winston.debug(that.name+": Testing filters for "+that._username(userId)+"/"+userId);
		...
		for(var x=0; x<that.options.banned.length;x++) {
			var banitem = that.options.banned[x].toLowerCase();
			if(msg.indexOf(banitem)>-1 || userId===banitem || toId===banitem) {
				return false;
			}
		}
		_.each(that.db, function(user) { //need to add a link checker in here. If there's a URL in the message, we should send it as a link, not a note.
			...
			for(x=0;x<user.keys.length;x++) {
				if(msg.indexOf(user.keys[x])>=0) {
					...
					if(user.pbApiKey) {
						that.winston.debug(that.name+" sending pushbullet for "+that._username(user.userId)+"/"+user.userId);
					...
					if(user.email) {
						...
						that.nodemailer.sendMail({
							...
							if(err) {that.winston.error("Error sending notification email for "+that._username(user.userId)+"/"+user.userId+": "+err);}
					...
					if(user.pouser && that.options.pushoverapikey) {
						...
						that.winston.debug(that.name+" sending pushover for "+that._username(user.userId)+"/"+user.userId);
						pusher.send({
							message: message,
							title: title
						}, function(err, result) {
							if(err) {
								that.winston.log(err);
