...
nodemailer = object - initialized nodemailer, overrides other mail options, allows usage of other transports (only smtp and direct are built-in)
...
saveTimer = int - frequently to write the database to disk (for the seen command). Set to -1 to disable.
*/
...
exports.create = function(name, chatBot, options) {
	...
	trigger.options.dbFile = trigger.options.hasOwnProperty('dbFile') ? trigger.options.dbFile : chatBot.username + '/' + "Notifications.db";
	...
	trigger.options.nodemailerOptions = trigger.options.nodemailerTransportOptions;
...
NotificationTrigger.prototype._onLoad = function() {
	...
	if(this.options.nodemailer) {
		that.winston.debug("Using passed-in nodemailer");
		this.nodemailer = this.options.nodemailer;
	} else if(this.options.nodemailerTransport && this.options.nodemailerTransportOptions && this.options.address) {
		try {
			that.winston.debug("Attempting to create nodemailer transport");
			if(this.options.nodemailerTransportOptions) {
				var smtpPool = require('nodemailer-smtp-pool');
				this.nodemailer = nodemailer.createTransport(smtpPool(this.options.nodemailerTransportOptions));
			} else {
				var directTransport = require('nodemailer-direct-transport');
				this.nodemailer = nodemailer.createTransport(directTransport({name:this.options.hostname||"steam-chat-bot"}));
			}
		} catch(err) {
			that.winston.error("Failed to create nodemailer transport");
	...
	this.hashfunc = this.options.hashfunc || function(address) {
		...
		shasum.update(that.name+salt+address); //don't let users guess the verification code.
		return shasum.digest('base64').substring(5,15); //keep it short (sha256 is too long).
	}

	if(that.options.saveTimer!=-1) {
		var timerid = setInterval(function(){
			try {
				that.winston.debug(that.name+": Writing database to disk");
				fs.writeFileSync(that.options.dbFile,JSON.stringify(that.db)); //this should be changed to a database soon.
			} catch(err) {
				that.winston.error(that.name+": Error saving database to disk:");
				that.winston.error(err.stack);
			}
		},that.options.saveTimer);
		this.winston.silly(this.name+": saveTimer id: "+timerid);
	}
}
...
NotificationTrigger.prototype._respond = function(toId,userId,message) {
	...
	if(userId!=toId) this.db[userId].seen = new Date().now(); //Also, since we have objects for everyone we can add a !seen function. Woohoo!
	if(this._stripCommand(message,this.options.seenCommand)) {
		...
		if(line == "delete yes") {
			that.db[userId] = { userId:userId, name:that._username(userId) };
		...
		} else if(line == "devices" && that.db[userId] && that.db[userId].pbApiKey) {
			var pusher = new PushBullet(that.db[userId].pbApiKey);
			pusher.devices(function(error, response){
				that.winston.silly(devices);
				if(error) {
			...
			if(email=="no" || email=="n" || email=="f" || email=="false") {
				that.winston.debug("Disabling emails for "+that._username(userId)+"/"+userId);
				...
				that.winston.info("Sending an email verification request for "+that._username(userId));
				that.winston.debug("Sending a verification request to "+email); //don't put email addresses in log unless we're debugging.
				that.nodemailer.sendMail({
					...
					if(err) {
						that.winston.error("Error sending email:");
						that.winston.error(err.message);
			...
			if(sendpm=="yes" || sendpm=="y" || sendpm=="t" || sendpm=="true") {
				that.winston.silly("PM enabled for "+that._username(userId)+"/"+userId+"'s notifications");
				...
				that.winston.silly("PM disabled for "+that._username(userId)+"/"+userId+"'s notifications");
			...
			that.winston.debug(that.name+": Testing filters for "+that._username(user.userId)+"/"+user.userId));
			that._respond('TEST NOTIFICATION FILTER',userId,'NotifyTest:'+message); //just call _respond on the same line
		} else {
			that._sendMessageAfterDelay(userId,"Either you're sending an invalid command, or I don't have an API key for you!\nPlease see https://efreak.github.com/node-steam-chat-bot/TRIGGERS.md#notificationtrigger for help.");
		}
		fs.writeFileSync(this.options.dbFile,JSON.stringify(this.db)); //this should be changed to a database soon.
		return true;
	} else {
		...
		_.each(that.db, function(user) { //need to add a link checker in here. If there's a URL in the message, we should send it as a link, not a note.
			...
			for(x=0;x<user.keys.length;x++) {
				if(msg.indexOf(user.keys[x])>=0) {
					...
					if(user.pbApiKey) {
						that.winston.debug(that.name+" sending pusbullet for "+that._username(user.userId)+"/"+user.userId);
						var pusher=new PushBullet(user.pbApiKey);
						pusher.note(user.deviceName,title,message);
					}
					var emailfail = false;
					if(user.email) {
						that.winston.debug(that.name+" sending email for "+that._username(user.userId)+"/"+user.userId);
						that.nodemailer.sendMail({
							from: that.options.address,
							to: user.email,
							subject: title,
							...
							that.winston.error("Error sending notification email for "+that._username(user.userId)+"/"+user.userId+": "+err.message);
					...
					if(!user.sendpm===false || emailfail==true) {
						that.winston.debug(that.name+" sending pm for "+that._username(user.userId)+"/"+user.userId);
