...
path = string - where should github attach? (you might be running multiple bots on the same host. I am) Defaults to whatever you *name* the trigger. Path will be doubled because of the way this function works.
...
GithubTrigger.prototype._onLoad = function() {
	...
	try {
		...
		that.winston.error(that.chatBot.name+"/"+that.name);
	...
	this.addRouter(this.options.path).use(function(req, res) {
		that.winston.silly(this.chatBot.name+"/"+this.name+": Request inbound");
		that.handler(req, res, function(err) {
			that.winston.silly(this.chatBot.name+"/"+this.name+": 404");
			res.statusCode = 404;
			res.end('no such location')
		});
	});
	this._callIfEnabled('*', function(event) {
		that.winston.silly(event);
	});
...
GithubTrigger.prototype._checkIfEnabled = function(hook, callback) {
