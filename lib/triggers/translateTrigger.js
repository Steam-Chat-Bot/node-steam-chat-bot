	...
	if(query && query.params.length === 4) {
		request.get({method: 'GET', encoding: 'utf8', uri: 'http://hablaa.com/hs/translation/' + query.params[1] + '/' + query.params[2] + '-' + query.params[3] + '/', json: true, followAllRedirects: true}, function(error, response, body) {
			if(error) {
				...
				that.winston.error('Code ' + response.statusCode + ' received from hablaa.com');
				return;
			}
			else {
				...
				if(result === undefined) {
					that._sendMessageAfterDelay(toId, 'Either ' + query.params[1] + ' is not a word/not defined in that language, or one of the languages you specified is incorrect. Please visit http://hablaa.com/hs/translation/' + query.params[1] + '/' + query.params[2] + '-' + query.params[3] + '/ for more information.');
				}
				else {
		...
		this._sendMessageAfterDelay(toId, '"!translate hello eng spa" will return "Hola".');
		return true;
	}
...
TranslateTrigger.prototype._getExampleTranslations = function(toId, userId, params) {
	...
	request.get({
		...
		if(error) {
			...
			if(body.length === 0) {
				that._sendMessageAfterDelay(toId, 'Body is empty (no example found). Please visit http://hablaa.com/hs/translations-examples/' + params[1] + '/' + params[2] + '-' + params[3] + '/ for more information.');
			}
			else {
				if(params[4] === undefined) {
					that._sendMessageAfterDelay(toId, 'You did not supply an example language.');
					that.winston.warn('No example language defined');
				}
				else if(params[4] === params[2]) {
					if(body[0].src === undefined) {
						that._sendMessageAfterDelay(toId, 'An example of this word in that language was not found. Please visit http://hablaa.com/hs/translations-examples/ for more information.');
					}
					else {
