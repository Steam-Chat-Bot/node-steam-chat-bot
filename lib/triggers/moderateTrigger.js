...
exports.create = function(name, chatBot, options) {
		...
		trigger.options.commands = trigger.options.commands || "!mod";
...
ModerateChatTrigger.prototype._respond = function(roomId, userId, message) {
		...
		if(query) {
			if (this.options.privatemessage && query != roomId)
				this.chatBot.sendMessage(query, "I have been told to moderate this chat by " + this.chatBot.users[userId].playerName + " (" + userId + ").";
			this.chatBot.setModerated(query);
			return true;
		} else {
			if(this.options.publicmessage)	this.chatBot.sendMessage(roomId, this.options.publicmessage);
			this.chatBot.setModerated(roomId);
			return true;
		}
		else return false;
}
