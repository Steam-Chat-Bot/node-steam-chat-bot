...
TumblrTrigger.prototype._respondToChatMessage = function(roomId, chatterId, message) {
	if (!this._checkUser(chatterId, this))
		return false;
	if (!this._checkRoom(roomId, this))
		return false;
	if(this._checkIgnores(chatterId,roomId, this))
		return false;
...
TumblrTrigger.prototype._performCommand = function(roomId, message, userId) {
	...
	switch (firstWord) {
		case postPhotoCommand: 
			return _postPhoto(linkData.link, linkData.context, this);
		case postLinkCommand: 
			return _postLink(linkData.link, linkData.context, this);
		case postAudioCommand: 
			return _postAudio(linkData.link, linkData.context, this);
		case postVideoCommand: 
			return _postVideo(linkData.link, linkData.context, this);
		case postQuoteCommand: 
			return _postQuote(otherWords, this);
		case postTextCommand: 
			return _postText(otherWords, this);
		case postChatCommand: 
			return _postChat(otherWords, this);
	}
...
TumblrTrigger.prototype._checkIgnores = function(toId, fromId, trigger) {
	if (!trigger.options.ignore || trigger.options.ignore.length == 0)
		return false;

	for (var i=0; i < trigger.options.ignore.length; i++)
		if (toId == trigger.options.ignore[i] || fromId == trigger.options.ignore[i])
			return true;
...
TumblrTrigger.prototype._checkRoom = function(toId, trigger) {
...
TumblrTrigger.prototype._checkUser = function(fromId, trigger) {
