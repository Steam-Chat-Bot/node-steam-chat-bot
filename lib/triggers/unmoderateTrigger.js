...
/*
Trigger that tells the bot to unmoderate a groupchat. Send a steamid64 or it will unlock the current chat.
command = string - a message must start with this + a space before a response will be given. Defaults to !unmod.
...
var UnmoderateChatTrigger = function() {
		UnmoderateChatTrigger.super_.apply(this, arguments);
};

util.inherits(UnmoderateChatTrigger, BaseTrigger);



var type = "UnmoderateChatTrigger";
exports.triggerType = type;
exports.create = function(name, chatBot, options) {
		var trigger = new UnmoderateChatTrigger(type, name, chatBot, options);
		trigger.options.commands = trigger.options.commands || "!unmod";
...
UnmoderateChatTrigger.prototype._respondToFriendMessage = function(userId, message) {
		if(this.options.allowprivate) return this._respond(null, userId, message); else return false;
}

// Return true if a message was sent
UnmoderateChatTrigger.prototype._respondToChatMessage = function(roomId, chatterId, message) {
		if(this.options.allowpublic) return this._respond(roomId, chatterId, message); else return false;
}

UnmoderateChatTrigger.prototype._respond = function(roomId, userId, message) {
		var query = this._stripCommand(message);
		if(query) {
			if (this.options.privatemessage && query != roomId)
				this.chatBot.sendMessage(query, "I have been told to unmoderate this chat by " + this.chatBot.users[userId].playerName + " (" + userId + ").";
			this.chatBot.setUnmoderated(query);
			return true;
		} else {
			if(this.options.publicmessage)	this.chatBot.sendMessage(roomId, this.options.publicmessage);
			this.chatBot.setUnmoderated(roomId);
			return true;
		}
		else return false;
}

UnmoderateChatTrigger.prototype._stripCommand = function(message) {
