...
UnmoderateChatTrigger.prototype._respond = function(roomId, userId, message) {
		...
		if(query) {
			if (this.options.privatemessage && query != roomId)
				this.chatBot.steamClient.sendMessage(query, "I have been told to unmoderate this chat by " + this.chatBot.steamClient.users[userId].playerName + " (" + userId + ").");
			this.chatBot.steamClient.setUnmoderated(query);
			return true;
		} else {
			if(this.options.publicmessage)	this.chatBot.steamClient.sendMessage(roomId, this.options.publicmessage);
			this.chatBot.steamClient.setUnmoderated(roomId);
			return true;
		}
		return false;
}
