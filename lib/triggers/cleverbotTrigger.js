...
CleverbotTrigger.prototype._respond = function(toId, message) {
	...
	if (message) {
		winston.info("Sending cleverbot request with params:");
		winston.info(this.cleverbot.params);
		var that = this;
		this.cleverbot.write(message, function(response) {
			winston.info("Cleverbot responded with params:");
			winston.info(that.cleverbot.params);
