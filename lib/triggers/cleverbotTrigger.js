...
exports.create = function(name, chatBot, options) {
	...
	if (trigger.options.cleverbot) {
		...
		if (trigger.options.session) {
			this.winston.info("Setting Cleverbot session: " + trigger.options.session);
	...
	if (message) {
		...
		Cleverbot.prepare(function() {
			that.cleverbot.write(message, function(response) {
				that.winston.info("Cleverbot responded with params:",that.cleverbot.params);
				if (response.message != '<html>' && response.message.trim() != '' && response.message.indexOf("Error:") != 0) {
					...
					that.winston.error("Cleverbot error:",response.message);
				}
			});
		});
