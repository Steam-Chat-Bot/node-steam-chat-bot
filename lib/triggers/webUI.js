...
admins        = Array     - []                        - Array of steamids of those allowed to access the webui
...
//		res.send(JSON.stringify(req.user));
	...
	this.sockets.on('connection', function(socket) {
		socket.emit('ROOMS',JSON.stringify(that.chatBot.steamFriends.chatRooms));
...
WebUI.prototype._respondToEnteredMessage = function(roomId,userId) {
	this.sockets.emit('ROOMS',JSON.stringify(this.chatBot.steamFriends.chatRooms));
	return false;
}
...
WebUI.prototype._announce = function(body) {
	...
	try {
		...
		console.log(success);
		success = this._makeAnnouncement2(body.target,body.headline,body.body,body.source);
		console.log(success);
	} catch(err) {
		return "Unknown error. Please check logs";
	}
...
//	this.sockets.emit('TRIGGERS',that._getTriggers());
}
...
//	this.sockets.emit('IGNORES',that.chatBot.ignores);
}
...
WebUI.prototype._makeAnnouncement2 = function(target, head, body, source) {
	var that = this;
	try {
		var post_data = {
			"sessionid" : that.chatBot.steamTrade.sessionID,
//			"sessionID" : that.chatBot.steamTrade.sessionID,
		...
		var uri = 'https://steamcommunity.com/groups/'+target+'/announcements/create?'+qs.stringify(post_data);
		that.winston.debug('POSTING announcement');
		return request({
			method:'POST',
			url:uri,
			jar:that.chatBot.steamTrade._j,
			headers:{
				'Content-Type': 'application/x-www-form-urlencoded',
				'Content-Length': post_data.length
			}
