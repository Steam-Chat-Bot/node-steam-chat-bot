...
UnlockChatTrigger.prototype._respond = function(roomId, userId, message) {
		...
		if(query) {
			if (this.options.privatemessage && query != roomId)
				this.chatBot.sendMessage(query, "I have been told to unlock this chat by " + this.chatBot.users[userId].playerName + " (" + userId + ").";
			this.chatBot.unlockChat(query);
			return true;
		} else {
			if(this.options.publicmessage)	this.chatBot.sendMessage(roomId, this.options.publicmessage);
			this.chatBot.unlockChat(roomId);
			return true;
		}
		else return false;
}
