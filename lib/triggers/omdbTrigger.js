...
OMDBTrigger.prototype._respond = function(roomId, userId, message) {
	...
	var that = this;
	if(query) {
		omdb.get(query,function(err,movie){
			if(err) {
				that._sendMessageAfterDelay(roomId, "Error!");
				that.winston.error("ERROR: " + err);
			...
			if(movie.imdb && movie.imdb.id) {
				result += "\nFor more information, visit imdb.com/title/" + movie.imdb.id;
			}
...
OMDBTrigger.prototype._stripCommand = function(message) {
