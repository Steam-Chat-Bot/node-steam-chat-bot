...
exports.create = function(name, chatBot, options) {
	...
	trigger.options.joinAll = options.admins || false;
	trigger.options.defaultMessage = options.defaultMessage || "Hello! I was invited by $invitername!";
	trigger.respectsMute = false;
	return trigger;
};

AcceptChatInviteTrigger.prototype._respondToChatInvite = function(roomId, roomName, inviterId) {
	if (roomId in this.options.chatrooms || this.options.joinAll === true || inviterId in this.options.joinAll) {
		...
		if (welcomeMsg) {
			welcomeMsg = welcomeMsg.split('$inviterurl').join('http://steamcommunity.com/profiles/'+inviterId);
