...
exports.create = function(name, chatBot, options) {
	var trigger = new WikiBotTrigger(type, name, chatBot, options);
		...
		trigger.winston = trigger.winston || trigger.chatBot.log;
...
WikiBotTrigger.prototype._logIn = function(toId, callback) {
	...
	var afterwhoami = function(username){
		if(that.options.wikiUsername==username){
			that.winston.info(that.name+": Already logged in.");
			if(callback && typeof callback=="function") callback(true);
			else that._sendMessageAfterDelay(toId,"Already logged in.");
		} else {
			var afterlogin = function(username){
				that.winston.info(that.name+": Logged in as "+username);
				if(callback && typeof callback=="function") callback(username);
				else that._sendMessageAfterDelay(toId,"Logged in as "+username);
			}
			var loginerr = function(err){
				that.winston.err(that.name+": "+err.stack)
				if(err.message=="NotExists") that._sendMessageAfterDelay(toId,"Error logging in: Wrong username. Aborting.");
				else if(err.message=="WrongPass") that._sendMessageAfterDelay(toId,"Error logging in: Wrong password. Aborting.");
				else that._sendMessageAfterDelay(toId,"Unknown error logging in. See log for details. Aborting.");
			}
...
WikiBotTrigger.prototype._redirectPage = function(toId,steamId,message) {
	...
	if(params.length < 2) {
		this._sendMessageAfterDelay(toId,"You need to specify the following: \"" +that.options.commandMove+" OldName||NewName[||Summary]\"");
		return;
	}
	var from = params[0];
	var to = params[1];
	...
	var afterlogin = function(username){
		if(username===true) that._sendMessageAfterDelay(toId,"Already logged in");
		else if(username) that._sendMessageAfterDelay(toId,"Logged in as "+username);
		var afteredit = function (title, revision, date) {
			if(revision && date && title) {
				...
				that.winston.info(that.name+": Revision #" + revision + " completed at " + date.toString() +". "+from+" redirected to "+to+".");
			} else {
				that._sendMessageAfterDelay(toId,"Page for "+result.appid+"/"+result.gamename+" not edited. Current page is the same.");
				that.winston.debug(that.name+": Page for "+result.appid+"/"+result.gamename+" not edited. Current page is the same.");
			}
		}
		var editerr = function(error){
			that._sendMessageAfterDelay(toId,"Failed to redirect "+from+" to "+to+". More info in log.");
			that.winston.err(that.name+": "+error.stack);
		}
...
WikiBotTrigger.prototype._editPage = function(toId,steamId,query) {
	...
	var afterlogin = function(username){
		...
		var afteredit = function (title, revision, date) {
			...
			that.winston.info(that.name+": Revision #" + revision + " completed at " + date.toString()+". "+title+" edited.");
		}
		var editerr = function(error){
...
WikiBotTrigger.prototype._importGames = function(toId,steamId,query) {
	...
	var afterSteamFetch = function(error, response, body) {
		if (error) {
			try {
				that.winston.warn("Code " + response.statusCode + " from steam");
				that._sendMessageAfterDelay(toId, "Code " + response.statusCode + " from steam");
			} catch (err) {that.winston.warn(err) }
			return;
		}
		var info = body;
		var afterlogin = function(username){
					...
					if(result.success) {
						var afterimport = function (title, revision, date) {
							if(revision && date && title) {
								...
								that.winston.info(that.name+": Revision #" + revision + " completed at " + date.toString() + ". Data imported for "+result.appid+"/"+result.gamename);
							} else {
								that._sendMessageAfterDelay(toId,"Page for "+result.appid+"/"+result.gamename+" not edited. Current page is the same.");
								that.winston.debug(that.name+": Page for "+result.appid+"/"+result.gamename+" not edited. Current page is the same.");
							}
							if(result.gamename.substring(1).toLowerCase()!=result.gamename.substring(1)) {
								var afteredit = function(redtitle, redrevision, reddate){
									if(redrevision) {
										...
										that.winston.info(that.name+": Revision #" + redrevision + " completed at " + reddate.toString()+". "+result.gamename.toLowerCase()+" redirected to "+result.gamename+".");
									} else {
										that._sendMessageAfterDelay(toId,"Lowercase redirect for "+result.appid+"/"+result.gamename+" not created. Already exists.");
										that.winston.debug(that.name+": Lowercase redirect for "+result.appid+"/"+result.gamename+" not created. Already exists.");
									}
								}
								var editerr = function(rederr){
									...
									that.winston.error(that.name+": "+rederr.stack);
								}
								...
								that.winston.info(that.name+": Redirecting game title "+result.gamename.toLowerCase()+" to "+result.gamename+"; would overwrite main game page.");
							}
							var afteredit = function(redtitle,redrevision,reddate){
								if(redrevision) {
									...
									that.winston.info(that.name+": Revision #" + redrevision + " completed at " + reddate.toString()+". "+result.appid+" redirected to "+result.gamename+".");
								} else {
									that._sendMessageAfterDelay(toId,"Appid Redirect for "+result.appid+"/"+result.gamename+" not created. Already exists.");
									that.winston.debug(that.name+": Appid Redirect for "+result.appid+"/"+result.gamename+" not created. Already exists.");
								}
							}
							var editerr = function(rederr){
