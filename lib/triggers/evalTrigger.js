...
WARNING: Please note that while the default is to run *outside* of the current context, this is still unsafe. Running it in the current global context will allow you to modify the bot objects, which can be dangerous, but also useful--you can load triggers, etc.
...
evalOpts.displayErrors = bool(true) - whether or not to print any errors to stderr, with the line of code that caused them highlighted, before throwing an exception. Will capture both syntax errors from compiling code and runtime errors thrown by executing the compiled code.
...
EvalTrigger.prototype._respond = function(roomId, userId, message) {
	...
	if(!this.context) {
		this.context = vm.createContext({bot:this.chatBot,trigger:this,global:global});
	}
	that.winston.info(that.chatBot.name+"/"+that.name+": Evaluating for user...");
	try {
		if(that.options.evalUnsafe) {
			var vmResult = vm.runInContext(cmd,that.context,that.options.evalOpts);
		} else {
			var vmResult = vm.runInNewContext(cmd,that.options.evalOpts);
		}
		console.log(vmResult);
		that._sendMessageAfterDelay(roomId,vmResult);
	} catch(err) {
