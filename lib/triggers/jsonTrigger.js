...
url = function(userId, roomId, message, trigger) - a function that creates the url. If you want to add the user's profile name, etc. roomId = userId if sent in private. The trigger allows access to the trigger constructor (trigger.name, trigger.options, trigger.chatBot, trigger._sendMessageAfterDelay may be useful). function(u,r,m,t){return "http://api.icndb.com/jokes/random/"+t._stripCommand(m)} or  function(u,r,m,t){return "http://steamcommunity.com/profiles/"+u+"?xml=1";}
parser = function(data, userId, roomId, message, trigger) - A function to parse the results and return the correct data (or none at all if you send the message yourself). if results are {"value": { "joke": "iwantthis" } } = function(d,a,b,c,d){return data.value.joke} = function(d, u, r, m, t){t._sendMessageAfterDelay(d.value.joke); return false}
...
JsonTrigger.prototype._respond = function(userId, toId, message) {
	if(this._stripCommand(message) && this.options.url && (this.options.parser instanceof Array || typeof this.options.parser=="function")) {
		...
		else if(typeof this.options.url === "string") fullurl = this.options.url;
		else return false;
		var that = this;
		try {
				...
				if(body) {
					try {
						if(that.options.parser instanceof Array) {
							for(var a=0;a<that.options.parser.length;a++) result = result[that.options.parser[a]];
							...
							if(result==true && result !==true) that._sendMessageAfterDelay(result);
						}
					} catch(err) {
