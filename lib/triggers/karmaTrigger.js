...
KarmaTrigger.prototype._respondToChatMessage = function(roomId, chatterId, m) {
	...
	if(cmd) {
		this._dbOperation('get',{key:cmd,callback:function(err,value){
			if(err) {
			...
			that._sendMessageAfterDelay(roomId,"'"+cmd+"' has "+value+"karma.");
		}});
		return true;
	}
	if(m.length>2 && m.length < this.options.maxlength && m.indexOf("++")==m.length-2) {
		this._changeScore(m.substr(m.length-2).replace(/\s/g,''),-1);
	}
	if(m.length>2 && m.length < this.options.maxlength && m.indexOf("--")==m.length-2) {
		this._changeScore(m.substr(m.length-2).replace(/\s/g,''),1);
	}
...
KarmaTrigger.prototype._changeScore = function(key,amount,cb) {
	...
	var score = this._dbOperation('get',{key:key,callback:callback});
}
