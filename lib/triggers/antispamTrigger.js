...
timers.messages - int(5*1000=5sec) = timeout (in ms) for messages. The bot will not warn or tattle on a user more than once every X milliseconds. Set explicitly to false to disable timeouts. Timeouts for warning/tattling are separate.
...
AntispamTrigger.prototype._respond = function(toId,userId,message) {
	...
	if(this.groups[toId][userId] >= this.options.score.warn && this.groups[toId][userId] <= this.options.score.warnMax) {
		if(!this.groups[toId][userId+"warned"]) {
			...
			this.winston[this.options.logLevel](this.chatBot.name+"/"+this.name+": NOT warning "+this._username(userId)+" for spamming in http://steamcommunity.com/gid/"+toId+" to prevent spam (timeout)");
		}
	} else if (this.groups[toId][userId] >= this.options.score.warn) {
		this.winston[this.options.logLevel](this.chatBot.name+"/"+this.name+": NOT warning "+this._username(userId)+" for spamming in http://steamcommunity.com/gid/"+toId+" to prevent spam (counter)");
	}
	...
	if(this.groups[toId][userId] >= this.options.score.tattle && this.groups[toId][userId] <= this.options.score.tattleMax) {
		if(!this.groups[toId][userId+"tattled"]) {
			...
			for(var admin in this.options.admins) {
				this._sendMessageAfterDelay(this.options.admins[admin],this._username(userId)+" is spamming in http://steamcommunity.com/gid/"+toId);
			}
			if(this.options.timers.messages) {
			...
			this.winston[this.options.logLevel](this.chatBot.name+"/"+this.name+": NOT tattling on "+this._username(userId)+" for spamming in http://steamcommunity.com/gid/"+toId+" to prevent spam (timeout)");
		}
	} else if(this.groups[toId][userId] >= this.options.score.tattle) {
		this.winston[this.options.logLevel](this.chatBot.name+"/"+this.name+": NOT tattling on "+this._username(userId)+" for spamming in http://steamcommunity.com/gid/"+toId+" to prevent spam (counter)");
	}
}
