...
SayTrigger.prototype._respond = function(roomId, userId, message) {
	...
	if(query && query.params[2]) {
		...
		toId = query.params.splice(0,1); //extract the steamid
		result = query.params.join(" "); //recombine the message
		this._sendMessageAfterDelay(toId, result); //send the message
		return true;
	}
	return false;
}
