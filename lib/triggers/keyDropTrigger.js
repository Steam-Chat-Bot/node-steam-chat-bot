...
exports.create = function(name, chatBot, options) {
	var trigger = new KeyDropTrigger(type, name, chatBot, options);
		...
		trigger.options.rndUserCommand = options.randomUserCommand || ["!random"];
		trigger.options.rndUserMessage = options.randomUserMessage || false;
...
KeyDropTrigger.prototype._respond = function(roomId, userId, message) {
	...
	if(this._stripCommand(message, this.options.randomUserCommand)) {
		var userid = this._randomProperty(this.chatBot.rooms()[this.options.room]);
		var username = this.chatBot.users()[userid].player_name;
		setTimeout(function(){
			if(that.options.randomUserMessage) {
				that._sendMessageAfterDelay(userid, that.options.randomUserMessage)
				that._sendMessageAfterDelay(that.options.room, username+ "/https://steamcommunity.com/users/"+userid+" has been randomly picked from the userlist and notified via PM");
			} else {
				that._sendMessageAfterDelay(that.options.room, username+ "/https://steamcommunity.com/users/"+userid+" has been randomly picked from the userlist.");
			}
		},that.options.timer);
		return true;
	}
