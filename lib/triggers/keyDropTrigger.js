...
exports.create = function(name, chatBot, options) {
	var trigger = new KeyDropTrigger(type, name, chatBot, options);
		...
		trigger.options.publicDelay       = options.publicDelay       || 5000;
		trigger.options.web               = options.web               || "/keys";
		trigger.options.namespace         = options.namespace         || trigger.options.web+Math.random().toString().substring(2); //undocumented: namespace for steam-mobile needs to be unique in global scope. Use a random number.
...
*/
	...
	app.get('/authenticate', steamlogin.authenticate(that.options.namespace), function(req, res) {
		that.winston.silly(that.logPrefix+'user',JSON.stringify(req.user));
	});
	app.get('/login', steamlogin.authenticate(that.options.namespace), function(req, res) {
		that.winston.silly(that.logPrefix+'user',JSON.stringify(req.user));
	});
...
//		res.send(JSON.stringify(req.user));
	...
	app.get('/logout', steamlogin.enforceLogin(that.options.namespace), function(req, res) {
		that.winston.silly(that.logPrefix+'user',JSON.stringify(req.user));
		req.logout();
		res.redirect('/keys/login');
	});
	app.get('/client', steamlogin.enforceLogin(that.options.namespace), function(req, res) {
		if(that.options.blocked.indexOf(req.user.steamid) > -1) {
			res.status(403).send("<html><body><h1>Access Denied</h1><h3>"+req.user.username+"/"+req.user.steamid+" is not allowed access");
			req.logout();
			...
			res.sendFile(__dirname+'/KeyDropTrigger/add.html');
		}
	});
	app.post('/addkey', steamlogin.enforceLogin(that.options.namespace), function(req,res){
		that.winston.silly(that.logPrefix+'user',JSON.stringify(req.user));
	...
	app.post('/delkey', steamlogin.enforceLogin(that.options.namespace), function(req,res){
		that.winston.silly(that.logPrefix+'user',JSON.stringify(req.user));
	...
	app.post('/getkeys', steamlogin.enforceLogin(that.options.namespace), function(req,res){
		that.winston.silly(that.logPrefix+'user',JSON.stringify(req.user));
		if(that.options.users.indexOf(req.user.steamid) === -1) {
	...
	app.post('/approvekey', steamlogin.enforceLogin(that.options.namespace), function(req,res){
		that.winston.silly(that.logPrefix+'user',JSON.stringify(req.user));
		if(that.options.users.indexOf(req.user.steamid) === -1) {
			that.winston.info('User '+req.user.username+'/'+req.user.steamid+' not allowed access!');
			res.json({error:req.user.username+"/"+req.user.steamid+" is not allowed access"});
	...
	app.post('/dropkey', steamlogin.enforceLogin(that.options.namespace), function(req,res){
		that.winston.silly(that.logPrefix+'user',JSON.stringify(req.user));
		if(that.options.users.indexOf(req.user.steamid) === -1) {
			that.winston.info('User '+req.user.username+'/'+req.user.steamid+' not allowed access!');
			res.json({error:req.user.username+"/"+req.user.steamid+" is not allowed access!"});
			...
			if(that._postKey(req.body.time||false,req.user.steamid,req.body.public)) {
				res.json({
					error: fnd.ws(that.options.dbFile,JSON.stringify(that.keys,null,4)) ? false : true,
					keys: that.keys
				});
	...
	app.get('/script.js', steamlogin.enforceLogin(that.options.namespace), function(req, res) {
		res.set('Content-Type', 'text/javascript');
		res.sendFile(__dirname+'/KeyDropTrigger/script.js');
	});
	app.get('/styles.css', steamlogin.enforceLogin(that.options.namespace), function(req, res) {
		if(that.options.users.indexOf(req.user.steamid) === -1) {
			res.set('Content-Type', 'text/css');
			res.sendFile(__dirname+'/KeyDropTrigger/add.css');
		} else {
			res.set('Content-Type', 'text/css');
...
KeyDropTrigger.prototype._getKeysForUser(who) {
