...
KeyDropTrigger.prototype._respond = function(roomId, userId, message) {
	...
	if (query && query.params[2]) {
		...
		if(!fnd.ws(this.options.dbFile,JSON.stringify(this.keys))) { //this should be changed to a database soon.
			...
			this.winston.error("Could not write key to disk",key);
		}
		return true;
	}
	...
	if (query) {
		...
		setTimeout(function(){
			...
			if(!fnd.ws(that.options.dbFile,JSON.stringify(that.keys))) { //this should be changed to a database soon.
				that._sendMessageAfterDelay(userId, "\""+key+"\" has been added to the end of the queue");
			} else {
				that._sendMessageAfterDelay(userId, "\""+key+"\" has been added to the end of the queue, however saving the queue to disk has failed. You may wish to ensure the key has been dropped later and/or posting it elsewhere.");
				that.winston.error("Could not write key to disk",key);
			}
		},that.options.timer);
