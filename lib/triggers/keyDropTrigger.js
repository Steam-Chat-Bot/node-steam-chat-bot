	...
	if(this._stripCommand(message, this.options.randomUserCommand)) {
		...
		setTimeout(function(){
			if(that.options.randomUserMessage) {
				...
				that._sendMessageAfterDelay(that.options.room, username+ "/https://steamcommunity.com/profiles/"+userid+" has been randomly picked from the userlist and notified via PM");
			} else {
				that._sendMessageAfterDelay(that.options.room, username+ "/https://steamcommunity.com/profiles/"+userid+" has been randomly picked from the userlist.");
			}
		},that.options.timer);
		return true;
	}
