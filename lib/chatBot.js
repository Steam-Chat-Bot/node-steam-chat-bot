...
var ChatBot = function(username, password, options) {
	...
	if (options.userFile) that.userFile == options.userFile;
	else that.userFile = that.username + ".users";
	this.log.debug("Loading userFile");
	if (fs.existsSync(this.userFile)) {
		this.users = JSON.parse(fs.readFileSync(serversFile));
	} else if(options.superadmin) {
		this.log.info("No userFile! using superadmin from config");
		this.users.superadmin = options.superadmin;
	} else this.log.warn("No userFile and no superadmin defined in config. etc etc");
...
ChatBot.prototype.connect = function() {
	...
	if (!this.connected) {
		try {
			...
			that.log.error("Exception trying to connect chat bot " + this.username, err);
		}
	}
}
...
*/
/*
ChatBot.prototype.addFlag = function(userID, newFlag, chatterID) { //use flags instead of permission levels or simple 'admin' and 'user' or even several permission levels. much nicer to work with.
	var that = this;
	if(!getuserflag(chatterid,'adduser')) {
		log();
		message("you can't do this");
	} else {
		if(!this.users.userID)
			this.users.userID={};
		this.users.userID.flags=newFlag;
		log();
		fs.writeFile(that.userFile, JSON.stringify(that.users));
	} else privmsg("You don't have permission for that!");
}

ChatBot.prototype.getUserFlag = function(userID,flag) { //for permissions that are global
	log();
	if(this.users.userID == this.users.superadmin, return true; //hardcoded superadmin can do *anything*
	if(this.users.userID.flag) return true;
}

ChatBot.prototype.getChannelFlag = function(userID,channel,flag) { //for permissions that are channel-specific
	log();
	if(this.users.userID == this.users.superadmin, return true;
	if(this.users.userID.flag) return true;
	if(this.users.userID.channels.flag) return true;
}

//flags: channeladmin (would be several channel-related flags), botadmin (multichannel admin), 
ChatBot.prototype.delUser = function(userID) {
