...
*/
	...
	this.steamClient.on("servers", function(servers) {
		try{
			that.winston.silly("Attempting to write servers file");
			fs.writeFileSync(serversFile, JSON.stringify(servers));
		} catch(err) {
			that.winston.error("Error writing servers file:");
...
ChatBot.prototype.addTrigger = function(name, type, options) {
	...
	try {
		winston.debug("Testing _onLoad for " + type + " trigger " + name);
		if (trigger && (!trigger._onLoad || trigger._onLoad())) {
			this.triggers[name] = trigger;
			return trigger;
		} else if (trigger) {
			winston.error("Error loading " + type + " trigger " + name);
			return null;
		}
	} catch(err) {
		winston.error("Error loading " + type + " trigger " + name + ":");
...
ChatBot.prototype.removeTrigger = function(name) {
	if (name in this.triggers) {
		winston.debug("Deleting trigger: "+name);
...
ChatBot.prototype.clearTriggers = function() {
	winston.debug("Clearing triggers");
	this.triggers = {};
}
...
ChatBot.prototype.setGames = function(appIdArray) {
	...
	if(this.games) this.winston.info("Playing gameIDs " + this.games.toString());
	else this.winston.info("Playing nothing");
...
ChatBot.prototype._onAnnouncement = function(groupID, headline) {
	this.winston.info(groupID + ' posted announcement with title ' + '"' + headline + '"');
	var haveEatenEvent = false;
	_.each(this.triggers, function(trigger) {
...
ChatBot.prototype._startWebServer = function(port){
	...
	try {
		this.server.listen(port,function(){winston.info('Webserver listening on '+port);});
		this.winston.debug("Webserver initialized successfully!")
	} catch(err) {
		winston.error(err)
		winston.error("ERROR starting the webserver. Most likely the port is already in use.")
	}
