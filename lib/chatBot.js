...
ChatBot.prototype.connect = function() {
	...
	if (!this.connected) {
		winston.info("Chatbot "+this.name+": Trying to connect");
		try {
			if(this.guardCode) {
			...
			winston.error("Chatbot "+this.name+": Exception trying to connect", err);
		}
	}
}
...
ChatBot.prototype.addTrigger = function(name, type, options) {
	...
	try {
		winston.debug("Chatbot "+this.name+": Testing _onLoad for " + type + " trigger " + name);
		if (trigger && (!trigger._onLoad || trigger._onLoad())) {
			this.triggers[name] = trigger;
			return trigger;
		} else if (trigger) {
			winston.error("Chatbot "+this.name+": Error loading " + type + " trigger " + name);
			return null;
		}
	} catch(err) {
		winston.error("Chatbot "+this.name+": Error loading " + type + " trigger " + name + ":");
...
ChatBot.prototype.removeTrigger = function(name) {
	if (name in this.triggers) {
		winston.debug("Chatbot "+this.name+": Deleting trigger: "+name);
...
ChatBot.prototype.clearTriggers = function() {
	winston.debug("Chatbot "+this.name+": Clearing triggers");
	this.triggers = {};
}
