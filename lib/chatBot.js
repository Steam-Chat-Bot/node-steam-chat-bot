var ver = "2.1.1-dev"; //update this before pushing to master.
...
ChatBot.prototype.joinGame = function(appId) {
	...
	this.steamUser.gamesPlayed([this.games]);
}
...
ChatBot.prototype.setGames = function(appIdArray) {
	...
	this.steamUser.gamesPlayed(this.games);	//play them!
}
...
ChatBot.prototype.setPrimaryGame = function(appId,delay) {
	...
	this.steamUser.gamesPlayed([appId]);		//first, play only this game, so it shows up
	var that = this;
	setTimeout(function(){
		that.winston.info("Chatbot "+that.name+": Playing gameIDs " + that.games.toString());
		that.steamUser.gamesPlayed(that.games);	//play them!
...
ChatBot.prototype.getSteamLevel = function(steamids) {
	...
	var that = this;
	this.steamFriends.getSteamLevel(steamids, function(levels) {
...
ChatBot.prototype._onLogOnResponse = function(res) {
	...
	if(res.EResult === steam.EResult.OK) {
		...
		this.steamWebLogon.webLogOn(function(sessionid, cookies) {
			...
			that.steamTrade.setCookie(cookies);
			that.winston.info('Logged into Steam web');
		});
	}
	else {
		this.logger.warn('EResult for logon response: ' + res.EResult);
	}
}
...
ChatBot.prototype._startWebServer = function(port){
	...
	//Eventually, we might want sockets for some kind of live chat function. This would be nicer than reloading the page or using xmlhttprequests. Also, someone might want them for something else? I dunno. In any case, this is better.
