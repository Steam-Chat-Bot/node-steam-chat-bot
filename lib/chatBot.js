...
//	this.steamTrade = new SteamTrade
	...
	if(!options.logFile || options.logFile != false)  {
		...
		that.winston.add(winston.transports.File, { level: options.logLevel||"info", colorize:false, timestamp:true, filename:that.logFile,json:false});
	}
	...
	if(options.sentryFile) {
		...
		if(fs.existsSync(options.sentryFile))
			...
			this.winston.warn("Sentry file defined in config does not exist. " + options.sentryFile + " will be created on successful login");
	} else {
		this.winston.info("Config file does not define a sentryfile. Attempting to autodetect sentry...");
		if(fs.existsSync("sentry."+this.username+".hash"))            { this.sentryFile = "sentry."+this.username+".hash";     this.winston.info("Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync("sentry."+this.username))             { this.sentryFile = "sentry."+this.username;             this.winston.info("Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync("sentryfile."+this.username))         { this.sentryFile = "sentryfile."+this.username;         this.winston.info("Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync("sentryfile."+this.username+".hash")) { this.sentryFile = "sentryfile."+this.username+".hash"; this.winston.info("Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync("bot."+this.username+".hash"))        { this.sentryFile = "bot."+this.username+".hash";        this.winston.info("Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync("bot."+this.username+".sentry"))      { this.sentryFile = "bot."+this.username+".sentry";      this.winston.info("Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync(this.username+".hash"))               { this.sentryFile = this.username+".hash";               this.winston.info("Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync(this.username+".sentry"))             { this.sentryFile = this.username+".sentry";             this.winston.info("Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync("sentry"))                            { this.sentryFile = "sentry"; this.winston.warn("Sentry file sentry detected, but it may be incorrect as it does not claim a username. If correct, please rename to bot."+this.username+".sentry or another support sentry filename, or rename and define file name in config.");
		} else { this.sentryFile = "bot."+this.username+".sentry"; this.winston.warn("Could not detect a sentryfile, and no guardCode defined. Using default sentryFile "+this.sentryFile+". I hope you have a guardCode defined or steamguard disabled. "); }
	}
...
ChatBot.prototype._onChatStateChange = function(stateChange, chatterActedOn, steamChatId, chatterActedBy) { 
	...
	if ((stateChange & steam.EChatMemberStateChange.Kicked) > 0) {
		...
		if(chatterActedOn==this.steamClient.steamId) this._removeChatFromAutojoin(steamChatId);

		var that = this;
		var haveSentMessage = false;
		_.each(this.triggers, function(trigger) {
			var sentMessageThisTrigger = trigger.onKickedChat(steamChatId, chatterActedOn, chatterActedBy, haveSentMessage, that.muted);
			haveSentMessage = haveSentMessage || sentMessageThisTrigger;
		});
	}
	...
	if ((stateChange & steam.EChatMemberStateChange.Entered) > 0) {
		...
		var haveSentMessage = false;
		_.each(this.triggers, function(trigger) {
			var sentMessageThisTrigger = trigger.onEnteredChat(steamChatId, chatterActedOn, haveSentMessage, that.muted);
			haveSentMessage = haveSentMessage || sentMessageThisTrigger;
		});
	}
	if ((stateChange & steam.EChatMemberStateChange.Left) > 0) {
		this.winston.info(this._userString(chatterActedOn) + " left " + steamChatId);

		var that = this;
		var haveSentMessage = false;
		_.each(this.triggers, function(trigger) {
			var sentMessageThisTrigger = trigger.onLeftChat(steamChatId, chatterActedOn, haveSentMessage, that.muted);
			haveSentMessage = haveSentMessage || sentMessageThisTrigger;
		});
	}
	if ((stateChange & steam.EChatMemberStateChange.Disconnected) > 0) {
		this.winston.info(this._userString(chatterActedOn) + " was disconnected from " + steamChatId);

		var that = this;
		var haveSentMessage = false;
		_.each(this.triggers, function(trigger) {
			var sentMessageThisTrigger = trigger.onDisconnected(steamChatId, chatterActedOn, haveSentMessage, that.muted);
			haveSentMessage = haveSentMessage || sentMessageThisTrigger;
		});
	}
	if ((stateChange & steam.EChatMemberStateChange.Banned) > 0) {
		this.winston.info(this._userString(chatterActedOn) + " was banned from " + steamChatId);

		var that = this;
		var haveSentMessage = false;
		_.each(this.triggers, function(trigger) {
			var sentMessageThisTrigger = trigger.onBannedChat(steamChatId, chatterActedOn, chatterActedBy, haveSentMessage, that.muted);
			haveSentMessage = haveSentMessage || sentMessageThisTrigger;
		});
	}
};
