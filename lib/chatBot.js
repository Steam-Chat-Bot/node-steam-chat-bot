...
var ChatBot = function(username, password, options) {
	...
	var babysitTimer = options.babysitTimer || 30*1000;
...
ChatBot.prototype._onFriendMsg = function(userId, message, type) { 
	...
	if (type == steam.EChatEntryType.ChatMsg) {
		...
		_.each(this.triggers, function(trigger) {
			haveSentMessage = trigger.onFriendMessage(userId, message, haveSentMessage);
		});
	}
};
...
ChatBot.prototype._onChatMsg = function(roomId, message, type, chatterId) { 
	...
	if (type == steam.EChatEntryType.ChatMsg) {
		...
		var haveSentMessage = false;
		_.each(this.triggers, function(trigger) {
			haveSentMessage = trigger.onChatMessage(roomId, chatterId, message, haveSentMessage, that.muted);
		});
	}
};
