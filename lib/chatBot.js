...
var ChatBot = function(username, password, options) {
	...
	if(!options.logFile || options.logFile !== false || options.logFile === null || options.logFile === undefined)  {
		...
		that.winston.info(that.name+"/chatBot.js: logging output to: " + that.logFile);
		that.winston.add(
	...
	if(options.sentryFile) {
		...
		if(fs.existsSync(options.sentryFile)) {
			this.winston.info(this.name+"/chatBot.js: using sentryfile as defined by options: "+options.sentryfile);
		} else {
			this.winston.warn(this.name+"/chatBot.js: Sentry file defined in config does not exist. "
				+ options.sentryFile + " will be created on successful login");
		}
	} else {
		this.winston.info(this.name+"/chatBot.js: Config file does not define a sentryfile. Attempting to autodetect sentry...");
		...
		if ("undefined" !== typeof sentryFile) {
			...
			this.winston.info(this.name+"/chatBot.js: Sentry file "+this.sentryFile+" detected.");
		} else if(fs.existsSync("sentry")) {
			this.sentryFile = "sentry";
			this.winston.warn(this.name+"/chatBot.js: Sentry file sentry detected, but it may be incorrect as it does not claim a username. If correct, please rename to bot."
			...
			this.winston.warn(this.name+"/chatBot.js: Could not detect a sentryfile, and no guardCode defined. Using default sentryFile "
...
ChatBot.prototype.logOn = function() {
	...
	if (!this.connected) {
		...
		try {
			...
			this.steamUser.logOn({ account_name: this.username, password: this.password,
				auth_code: this.guardCode, sha_sentryfile: sha });
		} catch (err) {
...
ChatBot.prototype._onLogOnResponse = function(res) {
	...
	if(res.eresult === steam.EResult.OK) {
		...
		this.winston.warn(this.name + '/chatBot.js: AccountLogonDenied (' + res.eresult + '): Please provide the guardCode sent to your email at ' + res.email_domain);
		process.exit(1);
	}
	else {
		this.winston.warn(this.name + '/chatBot.js: EResult for logon response: ' + res.eresult);
...
ChatBot.prototype._onUpdateMachineAuth = function(sentry, cb) {
	if(this.sentryFile) {
		...
		this.winston.info(this.name+"/chatBot.js: Obtained sentry. Writing to " + this.name + ".hash", sentry);
		fs.writeFileSync(this.name+"/bot." + this.name + ".sentry", sentry.bytes);
		cb({
