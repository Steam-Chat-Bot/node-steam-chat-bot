var ver = "2.4.0-dev"; //update this before pushing to master.
...
var ChatBot = function(options) {
	...
	//if we don't have an options object and we don't have connection info for discord or steam, output a warning and exit.
	} else if(!options || ((!options.steam || !options.steam.username || !options.steam.password) && (!options.discord||!options.discord.token))) {
		console.error("The options object is now *required*. Using a username/password combination is no longer supported."));
		console.error("You need to supply an options object in the following format, with at least one of the authentication methods (all other settings are optional)");
		console.error("options",{steam:{},discord:{},name:"",etc:""});
		process.exit();
...
*/
	...
	setInterval(function(){
		if(that.connected===true) {
			Object.keys(that.steamFriends.chatRooms).forEach(function(room){
				that.steamFriends.joinChat(room);
			});
		}
	}, that.options.rejoinTimer || 5000);
};
