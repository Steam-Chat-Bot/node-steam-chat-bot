...
ChatBot.prototype._onWebSessionID = function(sessionid) {
	...
	this.winston.debug('New sessionID: ', sessionid);
	var that = this;
	this.steamClient.webLogOn(function(cookie) {
		that.winston.debug('New cookie: ', cookie);
		that.cookie = cookie;
...
ChatBot.prototype._startWebServer = function(port){
	...
	try {
		server.listen(port,function(){winston.info('Webserver listening on '+port);});
	...
	var _getClientIp = this._getClientIp; //not sure what context this needs to be in, but I know this works...
	this.express.use(expressWinston.logger({
		winstonInstance: that.winston,
		msg: "HTTP {{res.statusCode}} {{_getClientIp(req)}} {{req.method}} {{res.responseTime}}ms {{req.hostname+req.originalUrl}}", // customize the default logging message.
