...
var ChatBot = function(username, password, options) {
	...
	this.steamClient.on('relationships', function(userId, relationship) { that._onRelationship(userId, relationship); });
...
// These events exist in node-steam, but are only half-implemented in this file...
	this.steamClient.on('loggedOff',  function() { that._onLoggedOff(); });
	this.steamClient.on('user', function(obscureData) {that._onUser(obscureData); });
	this.steamClient.on('friend', function(steamId,friendRelationship) {that._onFriend(steamId,friendRelationship); });
