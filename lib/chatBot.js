var ver = "2.3.0-dev"; //update this before pushing to master.
...
var ChatBot = function(username, password, options) {
	...
	if(fs.existsSync(this.sentryFile)) {
		...
		this.winston.warn(this.name+"/chatBot.js: sentryFile does not exist. "+options.sentryFile+" will be created on successful login");
	}
...
ChatBot.prototype.logOn = function() {
	...
	if (!this.connected) {
		...
		try {
			...
			this.steamUser.logOn({ account_name: that.username, password: that.password,
				auth_code: that.guardCode, sha_sentryfile: sha });
		} catch (err) {
